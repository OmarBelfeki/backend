...
let LegalService = class LegalService {
    ...
    async upsertLegal(createLegalDto) {
        const field = createLegalDto.type === 'privacy' ? 'privacyPolicy' : 'termsOfService';
        const id = 'singleton-legal-page';
        return this.prisma.legalPages.upsert({
            where: { id },
            update: {
                [field]: createLegalDto.content,
            },
            create: {
                id,
                [field]: createLegalDto.content,
    ...
    async findOne() {
        return this.prisma.legalPages.findFirst({
